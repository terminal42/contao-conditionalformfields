<?php

/**
 * Contao Open Source CMS
 * Copyright (C) 2005-2013 Leo Feyer
 *
 * Formerly known as TYPOlight Open Source CMS.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.
 *
 *
 * PHP version 5
 * @copyright  terminal42 gmbh 2013
 * @author     Yanick Witschi <yanick.witschi@terminal42.ch>
 * @license    http://opensource.org/licenses/lgpl-3.0.html
 */


/**
 * Fields
 */
$GLOBALS['TL_LANG']['tl_form_field']['isConditionalFormField']          = array('Nur mit erfüllter Bedingung anzeigen', 'Aktivieren Sie diese Checkbox, wenn Sie dieses Feld nur unter gewissen Bedingungen anzeigen möchten.');
$GLOBALS['TL_LANG']['tl_form_field']['conditionalFormFieldCondition']   = array('Bedingung', 'Schreiben Sie Ihre Bedingung wie Sie es in PHP tun würden. Bsp "$anderesfeld == \'de\' && $nocheinweiteres == \'test\')');
